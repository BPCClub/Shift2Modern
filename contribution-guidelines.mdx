...
<Card title="本指南使用 [*Docus*](https://docus.dev/) 构建，*Docus* 提供了许多额外组件并带入了 [*MDC Syntax*](https://content.nuxtjs.org/guide/writing/mdc/) 支持" icon="ic:baseline-settings">
...
> `.navigation.yml` 文件用于指定分类的标题，如果不指定则使用文件夹名作为标题。同时指定重定向点。 <br />
> 重定向点不需加索引号。

> #summary
> `content/2.sample-category-1/` 的 `.navigation.yml` 文件内容如下：
> #content
> ```yml
...
[Docus](https://docus.dev/introduction/writing-pages#frontmatter)
...
[Nuxtjs Content](https://content.nuxtjs.org/guide/writing/markdown#code-highlighting)
